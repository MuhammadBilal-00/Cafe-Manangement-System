...
{
    /// <summary>Phase 4: customer returns — restock + credit (reduces AR).</summary>
    ...
    public class SellReturnController : BaseController
    {
        ...
        public SellReturnController(ApplicationDbContext context, ISupplyChainService supply, IAuditLogService audit) : base(context)
        {
            _supply = supply;
            _audit = audit;
        }

        private record Line(int inventoryItemId, decimal quantity, decimal unitValue);

        public async Task<IActionResult> Index()
        {
            var branchIds = (await GetAccessibleBranches()).Select(b => b.Id).ToList();
            var returns = await _context.SellReturns.Include(r => r.Branch).Include(r => r.Customer).Include(r => r.Lines)
        ...
        public async Task<IActionResult> GetItems(int branchId)
        {
            ...
            var items = await _context.InventoryItems.Where(i => i.BranchId == branchId)
                .OrderBy(i => i.Name).Select(i => new { id = i.Id, name = i.Name, unit = i.Unit, cost = i.UnitPrice }).ToListAsync();
            return Json(items);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Create(int branchId, int? customerId, string? reason, string lines)
        {
            ...
            var ret = new SellReturn
            {
                BranchId = branchId, CustomerId = customerId, ReturnNumber = await GenAsync("SR", _context.SellReturns.Select(x => x.ReturnNumber)),
                Status = "Pending", TotalAmount = Math.Round(parsed.Sum(l => l.quantity * l.unitValue), 2), Reason = reason,
                CreatedById = GetCurrentUserId(), CreatedAt = DateTime.Now
            };
            _context.SellReturns.Add(ret);
            await _context.SaveChangesAsync();
            foreach (var l in parsed)
                _context.SellReturnLines.Add(new SellReturnLine { SellReturnId = ret.Id, InventoryItemId = l.inventoryItemId, Quantity = l.quantity, UnitValue = l.unitValue });
            await _context.SaveChangesAsync();
            await _audit.LogAsync("Create", "SellReturn", ret.Id, $"Sell return {ret.ReturnNumber}", branchId);
        ...
        {
            var ret = await _context.SellReturns.FirstOrDefaultAsync(r => r.Id == id);
            if (ret == null || !CanAccessBranch(ret.BranchId)) return Json(new { success = false, message = "Not found." });
            var (ok, msg) = await _supply.ApproveSellReturnAsync(id, GetCurrentUserId() ?? 0, HttpContext.Session.GetUserName() ?? "System");
            if (ok) await _audit.LogAsync("Approve", "SellReturn", id, "Approved", ret.BranchId);
